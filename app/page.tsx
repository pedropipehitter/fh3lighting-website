    ...
    <div className="min-h-screen">
      ...
      <section className="relative flex items-center justify-center min-h-[90vh] border-b border-neutral-800 overflow-hidden">
        <Image
          ...
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[#121212]/60 via-[#0d1a33]/50 to-[#121212]" />
        <div className="relative z-10 text-center px-6 max-w-3xl mx-auto">
          <p className="text-xs font-mono tracking-[0.3em] text-[#1e90ff] uppercase mb-6">
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-neutral-800">
          {sections.map(({ href, title, description }) => (
            ...
            >
              <h2 className="text-xl font-light text-white mb-3 group-hover:text-[#1e90ff] transition-colors">
                {title}
              </h2>
              <p className="text-sm text-neutral-500 leading-relaxed">{description}</p>
              <span className="inline-block mt-6 text-xs font-mono text-[#1e90ff] group-hover:text-[#1e90ff] transition-colors tracking-widest">
                EXPLORE →
              </span>
            </Link>
          ))}
